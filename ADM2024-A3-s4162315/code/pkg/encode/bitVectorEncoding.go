...
// encode binary f
/* to save the storage use bit to create the bitVector instead of byte, see the bitVectorEncoding_test.go file
rows := []string{
...
func bveInt16(rows [][]string) []byte {
	var packedData bytes.Buffer
	n := len(rows)
...
func bveInt32(rows [][]string) []byte {
	var packedData bytes.Buffer
	n := len(rows)
...
func bveInt64(rows [][]string) []byte {
	var packedData bytes.Buffer
	n := len(rows)
